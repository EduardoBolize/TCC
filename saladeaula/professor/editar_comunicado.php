<?php 
	...
	if(isset($_GET['cd']) and !empty($_GET['cd'])){
		...
		redirect("Favor, escolha um comunicado para editar se quiser acessar essa página!","crud_comunicado.php");
	}
	...
	if($query->num_rows > 0){
		...
		$row->dt_comunicado = formatar_para_br($row->dt_comunicado);
	}else{
...
<html style="user-select: none;">
	<head>
		<meta charset="utf-8">
		<title>Andorinha | Atualizar Comunicado</title>
			...
			<div class="col s12">
				<div class="col s10 offset-s1 z-depth-2" style="background-color:white; border-radius: 10px;margin-top:50px;padding-bottom:20px;">
					<div class="corpo_box col s10 offset-s1">
						<h3><?php echo "$row->tx_titulo em $row->dt_comunicado"; ?></h3>
